...
class LowLightDataset(Dataset):
    ...
    def __init__(self, tsv_file):
        ...
        self.data_pairs = pd.read_csv(tsv_file, sep='\t', header=None, names=['low_light', 'ground_truth'])
        
        # Check if files exist
        self.data_pairs = self.data_pairs[
            self.data_pairs.apply(lambda x: os.path.exists(x['low_light']) and os.path.exists(x['ground_truth']), axis=1)
        ]
